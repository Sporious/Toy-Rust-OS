...
pub fn spin_sleep_micros<T: Into<u64>>(i_micros: T) {
    let timer = SystemTimer::new();
    let start = timer.read();
    let micros = i_micros.into();
    while timer.read() < start + micros {}
}

pub fn spin_sleep_millis<T: Into<u64>>(i_millis: T) {
    spin_sleep_micros(i_millis.into() / 1000)
}
